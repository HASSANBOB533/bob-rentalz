...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ status, className = '', absolute = true }) => {
  ...
  // text color: white
  const layoutClasses = 'inline-flex items-center justify-center px-3 py-1.5 rounded-[20px] text-white text-xs font-medium shadow-sm';
  
  // Positioning
  const positionClasses = absolute ? 'absolute top-3 right-3 z-10' : '';

  return (
    <div className={cn(layoutClasses, bgColor, positionClasses, className)}>
      {status}
    </div>
  );
};
