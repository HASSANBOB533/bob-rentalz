...
export default function AdminPropertyDetailPage() {
  ...
  const mockProperties: Record<string, any> = {
    '101': {
      ...
      owner: { name: 'Ahmed Hassan', phone: '+20 100 123 4567', email: 'ahmed.hassan@email.com', joinedDate: 'Jan 2024' },
      city: 'New Cairo', area: '5th Settlement', propertyType: 'Villa',
      bedrooms: 4, bathrooms: 3, price: 45000,
      status: 'Pending', views: 0, leads: 0, assignedAgent: null, assignedTenant: null,
      amenities: ['Parking', 'Garden', 'Pool', 'Security', 'Gym'], submittedDate: '2 hours ago'
    },
    '103': {
      id: '103',
      name: 'Spacious Penthouse',
      description: 'Elegant penthouse with breathtaking city views.',
      owner: { name: 'Mohamed Ali', phone: '+20 100 234 5678', email: 'mohamed.ali@email.com', joinedDate: 'Feb 2024' },
      city: 'Zamalek', area: 'Nile View', propertyType: 'Penthouse',
      bedrooms: 3, bathrooms: 3, price: 35000,
      status: 'Approved', views: 45, leads: 8, assignedAgent: null, assignedTenant: null,
      amenities: ['Parking', 'Security', 'Elevator'], submittedDate: '1 day ago'
    },
    '106': {
      id: '106',
      name: 'Rented Villa in Katameya',
      description: 'Beautiful villa with established tenancy.',
      owner: { name: 'Nour Hassan', phone: '+20 100 999 8888', email: 'nour@email.com', joinedDate: 'Mar 2024' },
      city: 'New Cairo', area: 'Katameya', propertyType: 'Villa',
      bedrooms: 5, bathrooms: 4, price: 65000,
      status: 'Rented', views: 320, leads: 15, assignedAgent: 'Sarah Anderson', 
      assignedTenant: 'Tamer Hosny', assignedTenantId: '1',
      leaseStart: '2023-11-01', leaseEnd: '2024-11-01', // Past date for demo
      amenities: ['Pool', 'Garden', 'Security'], submittedDate: '1 year ago'
    }
  };
  ...
  const handleReject = () => {
    ...
    setProperty({
      ...property, 
      status: 'Rejected',
      rejectionDate: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
  ...
  const handleMarkAsRented = () => {
    ...
    setProperty({...property, status: 'Rented', leaseStart: '2024-11-20', leaseEnd: '2025-11-20'});
  ...
  const isLeaseEnded = property.status === 'Rented' && property.leaseEnd && new Date(property.leaseEnd) < new Date();
  ...
  return (
    <AdminDashboardLayout>
      <div className="px-4 sm:px-6 lg:px-8 py-8 max-w-6xl mx-auto">
        <div className="mb-6">
           <button onClick={() => navigate('/admin/properties')} className="flex items-center gap-2 text-[#0E56A4]">
        ...
        {property.status === 'Rejected' && (
          <div className="mb-6 bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3">
             ...
             <div className="flex-1">
                <h3 className="text-sm font-bold text-red-800">Rejected by Admin on {property.rejectionDate || 'Today'}</h3>
          ...
          <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
             ...
             <div className="flex-1">
                ...
                <p className="text-sm text-yellow-700 mt-1">
                   The lease for this property ended on <strong>{property.leaseEnd}</strong>. 
                   Please confirm if the tenant has moved out.
                </p>
             </div>
          </div>
        )}
           ...
           <div className="lg:col-span-2 space-y-6">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <div><p className="text-sm text-gray-500">Type</p><p className="font-medium">{property.propertyType}</p></div>
                    <div><p className="text-sm text-gray-500">Price</p><p className="font-medium text-[#0E56A4]">EGP {property.price.toLocaleString()}</p></div>
                    <div><p className="text-sm text-gray-500">Bedrooms</p><p className="font-medium">{property.bedrooms}</p></div>
                    <div><p className="text-sm text-gray-500">Bathrooms</p><p className="font-medium">{property.bathrooms}</p></div>
                 </div>
                       ...
                       <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
                          <div className="flex-1">
                             <div className="flex items-center gap-2 mb-1">
                                <span className="font-bold text-lg text-gray-900">{property.assignedTenant}</span>
                                <Badge className="bg-green-100 text-green-700 border-green-200">Rented</Badge>
                             </div>
                             <p className="text-xs text-gray-500 mb-3">Tenant ID: {property.assignedTenantId}</p>
              ...
              {property.status === 'Pending' && (
                 <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div className="flex gap-3 mb-4">
                       <Button onClick={handleApprove} className="flex-1 bg-[#0E56A4] text-white hover:bg-[#0A3F79]">Approve</Button>
                       <Dialog open={isRejectModalOpen} onOpenChange={setIsRejectModalOpen}>
                         <DialogTrigger asChild>
                           <Button className="flex-1 bg-[#D9534F] text-white hover:bg-[#C9302C]">Reject</Button>
                         </DialogTrigger>
                         <DialogContent>
                           <DialogHeader>
                             <DialogTitle>Reject Listing</DialogTitle>
                             <DialogDescription>
                               Are you sure you want to reject this listing? This action cannot be undone immediately.
                             </DialogDescription>
                           </DialogHeader>
                           <div className="py-4">
                           ...
                           <DialogFooter>
                             ...
                             <Button onClick={handleReject} className="bg-[#D9534F] text-white hover:bg-[#C9302C]">Submit</Button>
                           </DialogFooter>
                         </DialogContent>
                       </Dialog>
                    </div>
                 ...
                 <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    {property.assignedAgent ? (
                       <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                          ...
                          <Button variant="ghost" size="sm" className="text-[#0E56A4]">Change</Button>
                       </div>
                    ) : (
                       <div className="space-y-3">
                          ...
                          <Button onClick={handleAssignAgent} className="w-full bg-[#0E56A4]">Assign Agent</Button>
                       </div>
                    )}
                 </div>
              )}
              ...
              {property.status === 'Approved' && (
                 <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    {property.assignedTenant ? (
                       ...
                       <div className="space-y-3">
                          <p className="text-xs text-gray-500">Assign a prospect to this property to start the rental process.</p>
                          <Select value={selectedTenant} onValueChange={setSelectedTenant}>
                             <SelectTrigger><SelectValue placeholder="Select Prospect" /></SelectTrigger>
                             <SelectContent>
                                {mockProspects.map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}
                             </SelectContent>
                          </Select>
                          <Button onClick={handleAssignTenant} variant="outline" className="w-full border-[#0E56A4] text-[#0E56A4]">
                      ...
                      <Dialog open={isVacateModalOpen} onOpenChange={setIsVacateModalOpen}>
                        ...
                        <DialogContent>
                           <DialogHeader>
                              ...
                              <DialogDescription>
                                 Are you sure you want to mark this property as vacated? This will change the property status to available and archive the tenant's lease.
                              </DialogDescription>
                           </DialogHeader>
                           <DialogFooter>
                              <Button variant="outline" onClick={() => setIsVacateModalOpen(false)}>Cancel</Button>
                              <Button onClick={handleMarkAsVacated} className="bg-[#0E56A4] text-white">Confirm</Button>
                           </DialogFooter>
                        </DialogContent>
                      </Dialog>
                   )}
