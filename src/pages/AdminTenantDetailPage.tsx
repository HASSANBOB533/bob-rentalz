...
export default function AdminTenantDetailPage() {
  ...
  const mockTenants: Record<string, any> = {
    '3': {
      ...
      inquiries: [
        { id: '1', property: 'Modern 2BR Apartment', city: 'Maadi', status: 'Interested', date: 'Nov 15' },
      ]
    },
    ...
    '1': { // Rented - Lease Expiring Soon (Within 60 days)
      id: '1',
      name: 'Layla Hassan',
  ...
  const [documents, setDocuments] = useState([
    ...
    { id: 2, name: 'Lease_Agreement_Signed.pdf', type: 'Contract', date: '2025-11-15', uploadedBy: 'Admin' },
  ]);
  ...
  const isLeaseExpiringSoon = tenant.status === 'Rented' && daysUntilExpiry >= 0 && daysUntilExpiry <= 60;
  ...
  const archivedMessages: ChatMessage[] = [
    { id: 1, text: "Hello, when is the final inspection?", sender: "tenant", timestamp: "Nov 14, 2024 10:00 AM" },
    { id: 2, text: "Hi Khaled, we can schedule it for tomorrow at 2 PM.", sender: "agent", timestamp: "Nov 14, 2024 10:15 AM" },
    { id: 3, text: "That works for me, thanks.", sender: "tenant", timestamp: "Nov 14, 2024 10:20 AM" },
    { id: 4, text: "All keys were returned on 12 Jan 2025.", sender: "agent", timestamp: "Jan 12, 2025 4:00 PM" },
    { id: 5, text: "Thank you for the stay, we appreciate your tenancy.", sender: "agent", timestamp: "Jan 12, 2025 4:05 PM" }
  ];

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Rented': return <Badge className="bg-green-100 text-green-700">Rented</Badge>;
      case 'Approved': return <Badge className="bg-blue-100 text-blue-700">Approved</Badge>;
      case 'Past Tenant': return <Badge className="bg-gray-100 text-gray-700">Past Tenant</Badge>;
      default: return <Badge className="bg-yellow-100 text-yellow-700">Prospect</Badge>;
    }
  };
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-8 max-w-6xl mx-auto">
          ...
          <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
             ...
             <div className="flex-1">
                ...
                <p className="text-sm text-yellow-700 mt-1">
                   The lease period for this tenant has ended. You can now mark the tenancy as completed.
                </p>
             </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-1 space-y-6">
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                 <div className="flex flex-col items-center text-center mb-6">
                    <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-4 ${tenant.status === 'Past Tenant' ? 'bg-gray-200' : 'bg-[#E9C500]'}`}>
                       <UserCircle className={`w-12 h-12 ${tenant.status === 'Past Tenant' ? 'text-gray-500' : 'text-[#0E56A4]'}`} />
                    </div>
                 ...
                 <div className="space-y-4 border-t border-gray-100 pt-4">
                    ...
                    <div className="flex items-center justify-between text-sm">
                       <span className="text-gray-500 flex items-center gap-2"><User className="w-4 h-4"/> Agent</span>
           ...
           <div className="lg:col-span-2 space-y-6">
                 ...
                 {tenant.currentProperty ? (
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 flex justify-between items-center">
                       <div>
                          ...
                          <p className="text-sm text-[#0E56A4] font-medium">EGP {tenant.currentProperty.price.toLocaleString()}/mo</p>
                          {(tenant.status === 'Rented' || tenant.status === 'Past Tenant') && (
                             <div className="mt-1 text-xs text-gray-500 space-y-1">
                                <p>Lease Start: {tenant.leaseStart}</p>
                                <p className={tenant.status === 'Past Tenant' ? "text-red-600 font-medium" : ""}>Lease End: {tenant.leaseEnd}</p>
                             </div>
                          )}
                       </div>
                    ...
                    <div className="space-y-4">
                       {tenant.status === 'Past Tenant' ? (
                          ...
                          <>
                             <p className="text-sm text-gray-500">No property assigned yet. Select a property to proceed.</p>
                             <div className="flex gap-3">
                                ...
                                <Button onClick={handleAssignProperty} className="bg-[#0E56A4]">Assign</Button>
                             </div>
                          </>
                       )}
                    </div>
                           ...
                           <Dialog open={isRenewModalOpen} onOpenChange={setIsRenewModalOpen}>
                              ...
                              <DialogContent>
                                 ...
                                 <div className="grid gap-4 py-4">
                                    <div className="grid gap-2">
                                       ...
                                       <Input 
                                          ...
                                          onChange={(e) => setRenewForm({...renewForm, start: e.target.value})} 
                                       />
                                    </div>
                                    <div className="grid gap-2">
                                 ...
                                 <DialogFooter>
                                    ...
                                    <Button onClick={handleRenewLease} className="bg-[#0E56A4] text-white">Confirm</Button>
                                 </DialogFooter>
                              </DialogContent>
                           </Dialog>
                        </div>
                           ...
                           <Dialog open={isEndTenancyModalOpen} onOpenChange={setIsEndTenancyModalOpen}>
                              ...
                              <DialogContent>
                                 <DialogHeader>
                                    ...
                                    <DialogDescription>
                                       Are you sure you want to mark this tenancy as completed? This will archive the tenant and mark the property as vacant.
                                    </DialogDescription>
                                 </DialogHeader>
                                 <DialogFooter>
                                    <Button variant="outline" onClick={() => setIsEndTenancyModalOpen(false)}>Cancel</Button>
                                    <Button onClick={handleEndTenancy} className="bg-[#0E56A4] text-white">Confirm</Button>
                                 </DialogFooter>
                              </DialogContent>
                           </Dialog>
                        </div>
              ...
              {(tenant.status === 'Approved' || tenant.status === 'Rented' || tenant.status === 'Past Tenant') && (
                 <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div className="flex justify-between items-center mb-4">
                       ...
                       {tenant.status !== 'Past Tenant' && (
                          <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
                             <DialogTrigger asChild>
                                <Button size="sm" variant="outline" className="text-[#0E56A4] border-[#0E56A4]">
                             ...
                             <DialogContent>
                                <DialogHeader>
                                   ...
                                   <DialogDescription>Upload contracts, IDs, or proof of payment.</DialogDescription>
                                </DialogHeader>
