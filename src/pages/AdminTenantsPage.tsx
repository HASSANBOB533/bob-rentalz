...
export default function AdminTenantsPage() {
  ...
  const filteredTenants = mockTenants.filter(tenant => {
    ...
    const searchMatch = searchQuery === '' || 
      ...
      (tenant.currentProperty && tenant.currentProperty.toLowerCase().includes(searchQuery.toLowerCase()));
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {['all', 'prospects', 'approved', 'rented', 'past'].map((filter) => (
             ...
             >
               {filter === 'past' ? 'Past Tenants' : filter === 'prospects' ? 'Prospect Tenants' : filter === 'approved' ? 'Approved Tenants' : filter === 'rented' ? 'Rented Tenants' : 'All'}
             </button>
            ))}
          </div>
            ...
            filteredTenants.map((tenant) => (
              ...
              >
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                  {/* Tenant Avatar */}
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center flex-shrink-0 ${tenant.status === 'Past Tenant' ? 'bg-gray-200' : 'bg-[#E9C500]'}`}>
                    <UserCircle className={`w-10 h-10 ${tenant.status === 'Past Tenant' ? 'text-gray-500' : 'text-[#0E56A4]'}`} />
                  </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-3 mb-3 bg-gray-50 p-3 rounded-lg border border-gray-100">
                          <p className="text-xs font-semibold text-[#0E56A4] mb-2">
                             {tenant.status === 'Past Tenant' ? 'Previous Tenancy Information' : 'Reference Information'}
                          </p>
                          {tenant.currentProperty ? (
                            <div className="flex items-start justify-between gap-4">
                              <div>
                                ...
                                {tenant.status === 'Past Tenant' && (tenant.leaseStart || tenant.leaseEnd) && (
                            ...
                            <p className="text-sm text-gray-500 italic">No assigned property yet.</p>
                          )}
                        </div>
                      </div>
